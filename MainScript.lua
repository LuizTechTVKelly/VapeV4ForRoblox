...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		...
		suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/LuizTechTVKelly/VapeV4ForRoblox/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		...
		displayErrorPopup("Vape has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			...
			for i,v in pairs(listfiles("vape/CustomModules")) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/LuizTechTVKelly/VapeV4ForRoblox/"..readfile("vape/commithash.txt").."/CustomModules/"..last) end)
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/LuizTechTVKelly/VapeProfiles/main/Profiles/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/LuizTechTVKelly/VapeProfiles/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
    ...
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			...
			if shared.VapeDeveloper then 
				...
				loadstring(game:HttpGet("https://raw.githubusercontent.com/LuizTechTVKelly/VapeV4ForRoblox/"..readfile("vape/commithash.txt").."/NewMainScript.lua", true))() 
			end
		]]
		if shared.VapeDeveloper then
	...
	if not shared.VapeIndependent then
		...
		if isfile("vape/CustomModules/"..game.PlaceId..".lua") then
			...
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/LuizTechTVKelly/VapeV4ForRoblox/"..readfile("vape/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
